...
SongPlaylist.prototype.addEventEmitter = function () {
    ...
    function updateTime(time) {
        ...
        if (!songPlaylist.songModel) {
            return;
        }

        var instrumentsTiming = songPlaylist.songModel.getInstrumentsTiming(time);
        indicateStopStart(time, instrumentsTiming);
    }

    function indicateStopStart(time, instrumentsTiming) {

        $.each(instrumentsTiming.start, function (key, value) {
            var header = $container.find(".controls:eq(" + value.index + ") header");
            if (header.length) {
                header.removeClass("stop").addClass("start");
                console.log(time + " " + value.name + " start");
                doUnmute(value);
            }
        });

        $.each(instrumentsTiming.stop, function (key, value) {
            var header = $container.find(".controls:eq(" + value.index + ") header");
            if (header.length) {
                header.removeClass("start").addClass("stop");
                console.log(time + " " + value.name + " stop");
                doMute(value);
            }
        });
    }

    function doMute(value) {
        var btnMute = $container.find(".btn-mute:eq(" + value.index + ")");
    ...
    function resetMute() {
        $container.find(".controls header").removeClass("stop start");
    }
